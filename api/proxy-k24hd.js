...
<body class="text-slate-100 min-h-screen pb-16 px-4 sm:px-6">
    ...
    <div class="max-w-7xl mx-auto pt-8">
        ...
        <header class="text-center mb-8">
            <div class="inline-flex items-center gap-3 bg-slate-900/80 px-4 py-2 rounded-full border border-slate-700/50 mb-4">
                ...
                <span class="text-xs font-semibold text-slate-300 uppercase tracking-widest">Version 2.0 Pro</span>
            </div>
            ...
            <p class="text-slate-400 text-sm md:text-base max-w-2xl mx-auto font-light">
                ...
                ดึงข้อมูลทุกตอนอย่างเป็นระบบ พร้อมระบบพรีวิวดูสดในแดชบอร์ด
            </p>
        </header>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button onclick="copyAllM3UToClipboard()" class="bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-100 font-semibold px-4 py-2.5 rounded-xl flex items-center gap-2 transition-all">
                        <i class="fa-solid fa-copy"></i> คัดลอก M3U ทั้งหมด
                    </button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        // Native Web Audio API Synthesizer (Ultra-stable, zero scheduler conflicts)
        let audioCtx = null;
        ...
        function playTone(freq, duration, type = 'sine', delay = 0) {
            try {
                ...
                // Volume envelope to prevent popping/clicking noises
        ...
        async function request(url, isJson = false) {
            ...
            try {
                ...
                if (!res.ok) {
                    appendLog(`การเชื่อมต่อผ่านพร็อกซีล้มเหลว: HTTP ${res.status}`, 'text-amber-500');
                    return null;
                }
                return isJson ? await res.json() : await res.text();
            } catch (e) {
                appendLog(`เกิดข้อผิดพลาดในการดึงข้อมูล: ${e.message}`, 'text-rose-500');
                return null;
            }
        }

        async function getStream(postId) {
            const json = await request(encodeURI(`https://kubhd24.net/wp-admin/admin-ajax.php?action=mix_get_player&post_id=${postId}`), true);
        ...
        async function parseSeries(id) {
            // เข้ารหัส URL ด้วย encodeURI เพื่อแก้ปัญหาภาษาไทยที่ทำให้ Proxy หาหน้าเว็บไม่เจอ (HTTP 404)
            const html = await request(encodeURI(`https://kubhd24.net/series/${id}/`));
            if (!html) return null;
            ...
            let episodes = [];
            appendLog(`กำลังสืบค้นลิงก์สตรีมทั้งหมดจำนวน ${epButtons.length} ตอน ของเรื่อง "${title}"`, 'text-blue-300');
            
            for (const btn of epButtons) {
                if (isStopped) break;
                const url = await getStream(btn.id);
                if (url) {
                    episodes.push({ title: btn.textContent.trim(), url });
                    triggerSound('tick');
                }
            }
        ...
        async function startHarvesting() {
            ...
            appendLog(`[SYSTEM] เริ่มกระบวนการดาวน์โหลดข้อมูลหมวดหมู่: ${category} หน้าที่ ${startPage} ถึง ${endPage}...`, 'text-blue-400');
            ...
            for (let p = startPage; p <= endPage; p++) {
                ...
                appendLog(`กำลังเชื่อมต่อหน้าหมวดหมู่ที่ ${p}: ${pageUrl}`, 'text-slate-400');
                
                // เข้ารหัส URL หน้าหมวดหมู่ด้วยเช่นกันเพื่อความเสถียร
                const catHtml = await request(encodeURI(pageUrl));
                if (!catHtml) {
                    appendLog(`ไม่พบคอนเทนต์ในหน้าที่ ${p} หรือโหลดหน้าล้มเหลว`, 'text-amber-500');
                    continue;
                }
                ...
                if(ids.length === 0) {
                    appendLog(`ดึงรหัส URL ซีรีส์ไม่สำเร็จ หรือโครงสร้างหน้าเว็บอาจเปลี่ยนไปในหน้า ${p}`, 'text-amber-500');
                    continue;
                }

                totalSeriesFound += ids.length;
                stats.textContent = `กำลังประมวลผลหน้าที่ ${p} (พบสะสมแล้ว ${totalSeriesFound} เรื่อง)`;
                appendLog(`พบซีรีส์ทั้งหมด ${ids.length} เรื่องในหน้า ${p}`, 'text-emerald-400');

                for (let i = 0; i < ids.length; i++) {
                    if (isStopped) break;
                    
                    loader.textContent = `⏳ หน้าที่ ${p} | กำลังแกะข้อมูลเรื่องที่ ${i+1}/${ids.length}`;
                    appendLog(`[กำลังดึงรหัส]: ${ids[i]} (${i+1}/${ids.length})`, 'text-indigo-300');
                    
                    const data = await parseSeries(ids[i]);
                    if (data && data.episodes.length > 0) {
                        // Check if already in array to prevent duplicate representation
                        if(!gatheredSeries.some(s => s.id === data.id)) {
                            gatheredSeries.push(data);
                            renderCard(data, p);
                            totalEpisodesFound += data.episodes.length;
                        }
                        triggerSound('success');
                        showToast(`ดึงข้อมูล "${data.title}" สำเร็จ!`, 'success');
                    } else {
                        appendLog(`ข้ามข้อมูลเรื่อง ${ids[i]} เนื่องจากตรวจพบศูนย์ตอนสตรีม`, 'text-amber-500');
                    }
                }
            }
            ...
            if (isStopped) {
                loader.textContent = `⏹ หยุดกระบวนการชั่วคราวแล้ว`;
                appendLog(`⏹ กระบวนการหยุดโดยผู้ใช้ ได้รับไปทั้งหมด ${gatheredSeries.length} เรื่อง รวม ${totalEpisodesFound} ตอน`, 'text-rose-400 animate-pulse');
                showToast(`หยุดการดึงข้อมูลแล้ว`, 'error');
            } else {
                loader.textContent = `✅ สำเร็จเสร็จสิ้น!`;
                appendLog(`✅ สแกนสำเร็จครบถ้วน! สรุปผลดึงได้ ${gatheredSeries.length} เรื่องรวมยอดสะสม ${totalEpisodesFound} ตอนย่อย`, 'text-emerald-400 font-bold');
                showToast(`ดึงข้อมูลทั้งหมดเสร็จเรียบร้อย!`, 'success');
                triggerSound('done');
            }
        ...
        function stopHarvesting() {
            if (!activeHarvest) return;
            isStopped = true;
            appendLog(`[SYSTEM] มีคำสั่งร้องขอให้ยกเลิกปฏิบัติงาน...`, 'text-rose-500');
            document.getElementById('btnStop').disabled = true;
            document.getElementById('btnStop').textContent = "กำลังสั่งให้หยุด...";
        }
        ...
        function renderCard(data, pageNum) {
            ...
            // Generate Episodes Options List
            let optionsHtml = '';
        ...
        function compileMegaM3U() {
            ...
            gatheredSeries.forEach(series => {
                series.episodes.forEach(ep => {
                    megaM3U += `#EXTINF:-1 tvg-logo="${series.poster}" group-title="${series.title}", ${series.title} - ${ep.title}\n${ep.url}\n`;
                });
            });
            return megaM3U;
        }
        ...
        // Close modal when click outside of video block
